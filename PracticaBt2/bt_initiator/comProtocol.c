...
Protocol packageDecode(char *str){
    ...
    uint32_t crc32 = 0, crc32_recalculated = 0;
    ...
    //Verificacion de crc32
    char aux[255];
    int size = 0;
    array_append(aux, size++, (char) pkg.header);
    array_append(aux, size++, (char) pkg.command);

    array_append(aux, size++, (char)(pkg.length == 0x0 ? -0x1 : pkg.length));

    for(int i = 0; i < pkg.length; i++)
        array_append(aux, size++, (char)pkg.data[i]);
    array_append(aux, size++, (char) pkg.end);


    crc32_recalculated = crc32b(aux) >> 8;
    printf("%08X | %08X \n", pkg.crc32, crc32_recalculated);
    if(crc32_recalculated == pkg.crc32)
        return pkg;
    pkg.crc32 = -1;
    return pkg;
}
